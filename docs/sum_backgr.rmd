...
This function summarises many background files, best applied to summarise background that has been run for longer time (many measurement cycles). It becomes useful when trying to better understand system-wide respiration rate, across all channels. For example, it may be used to explore the variability of background respiration rates between different respirometry chambers,or to explore overtime changes in backgrounds respiration. This is shorthand way to get background MO2 <a style="color:grey">mgO2/L/min<a> through `MMR_SMR_AS_EPOC` function.
