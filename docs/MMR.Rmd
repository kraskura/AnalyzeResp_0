...
<div class="page-header">
  ...
  <p>Source: <a href = "#" style = "color:gray;">https://github.com/kraskura/AnalyzeResp_0/blob/main/docs/MMR.Rmd</a></p>
</div>
...
This function will take files with maximum metabolic rate recordings in up to 4 Channels/animals (8-channel FireSting has 4 oxygen sensor channels and 4 temperature probes). This function finds the highest metabolic rate section for each Channel within a specified measurement cycle; it is using a sliding window method (more in [Little et al 2020](https://doi.org/10.1086/708673)). The steepest regression slope is found for the following time periods: 60, 90, 120, and 180 s. The full length regression is always calculated. All non-MMR cycles are analyzed using full time period without sliding window approach.
...
**mmr_Ch1**           | A number indicated in which cycle the animal in FireSting Channel 1 is expected to reach its maximum metabolic rate. Must specify to analyze metabolic rate, or enter `0`, which will drop the channel from analysis. | `0`
...
**clean_Ch1**     | Specific to MMR cycle for each animal (or cycle). A time window (min) that specifies a quality section on metabolic rate measurement to be used. This may be used to "clean" the data. Default will use full length measurement cycle as set by `cycle_start` and `cycle_end` arguments. | `c(0,0)`
...
**path**          | Specify working directory for all output files. This argument has two options, i) files can be automatically organized in newly created local directories. Any character may be used. | `"."` (local directory)
...
Finding best representation of MMR. 
...
* ()_SLIDINGsetCh2.csv
  ...
  - Primary metrics included: regression parameters (slope = m, intercept = b, and R~2~) of each "sliding" slope, length (60 s, 90 s, 120 s, 180 s) timeframe of each slope, and temperature (ºC). 
* ()_analyzed.csv
  - Local dir(s): csv_analyzed (in two parent directories: MANUAL/csv_analyzed, MMR_SMR_AS_EPOC/csv_analyzed)
  - Primary metrics included: the steepest slope for each MMR timeframe(60 s, 90 s, 120 s, 180 s), respective regression parameters of those slopes, temparature; the same parameters are provided for the consecutive measurement cycles.

2. _.png files_
* ()_ChX.png  (X = the number of channel)
  - Local dir: channel_plots
  - Multipanel figure showing the MMR cycle and overlaid colored regression slopes with for all sliding windows 
* ()_SLIDING_MMRanalysisChX.png
  - Local dir: channel_plots_MMRanalysis
  - Analytical boxplot with all sliding slopes and their R~2~ plotted. This can be informative to select values best representing MMR of the animal. (literature on extended methods: [Little et al 2021, Physiological and Biochemical Zoology](https://doi.org/10.1086/708673))
...
1. MMR cycles: use arg `clean_ChX = c(,)`
2. The cycles: use arg `inv.data = "xyz.csv"`; external MMR cleaning or inventory datafile (.csv) to document and apply changes. 

We call these inventory files to create traceable and clear selection of all manually applied changes. Template can be found [here on GitHub page](herehere) 
...
`MMRslide` Arguments: <a style="color:grey"> d, Ch, data.MMR, r, r_temp, newdata_set, newdata_mmr </a>
`Channel_mmr` Arguments: <a style="color:grey">data.MMR, dataMMR, cycle_start, cycle_end, Ch_list, cycles, j, delays, rows, rows_temp, newdata_mmr, newdata_set</a>
...
- `ChX: INVENTORY data, adjusting non-MMR cycle times`: indicates which that channel (X) will contaon adjusted measurement timeframe for non-MMR cycles
...
[Little et al 2021, Physiological and Biochemical Zoology](https://doi.org/10.1086/708673)
